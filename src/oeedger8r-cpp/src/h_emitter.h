...
class HEmitter
{
  ...
  public:
    ...
    void emit_pop_t_h(const std::string& dir_with_sep = "")
    {
        gen_t_h_ = true;
   ...
   void emit_pop_h(const std::string& prefix = "")
   {
        out() << "// PopSGX autogenerated file - Carlos Bilbao bilbao {at} vt.edu";
        std::string guard =
            "EDGER8R_POP_" + upper(edl_->name_) + (gen_t_h_ ? "_T" : "_U") + "_H";
        std::string inc = (gen_t_h_ ? "enclave" : "host");
        header(out(), guard);
        out() << ""
              << "#include <openenclave/" + inc + ".h>"
              ...
              << "#include \"../popsgx.h\""
              << "#include \"" + edl_->name_ + "_args.h\"";
              if (edl_->name_.find("file") != std::string::npos && !gen_t_h_){
                out() << "#include <calls.h>"
                      << "#include <ecall_ids.h>";
              }
        ...
        if (!gen_t_h_){
                out() << "int pid_server;\n"
                      ...
                      << pop_create_prototype(edl_->name_) + ";"
                      << "";
        }
        out() << "/**** ECALL prototypes. ****/";
        trusted_prototypes(prefix);
