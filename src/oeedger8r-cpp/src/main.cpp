...
int main(int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        if (a == "--search-path")
            ...
            experimental = true;
        else if (a == "--pop-sgx" || a == "--popsgx" || a == "--PopSGX")
        {
            printf("Generating files for PopSGX...\n");
            pop_sgx = true;
        }
    ...
    for (std::string& file : files)
    {
        ...
        if (!pop_sgx){
            ...
            {
                ArgsHEmitter(edl).emit(trusted_dir);
                HEmitter(edl).emit_pop_t_h(trusted_dir);
                if (!header_only)
                    CEmitter(edl).emit_pop_t_c(trusted_dir);
            }
            if (gen_untrusted)
            {
                std::string prefix = use_prefix ? (edl->name_ + "_") : "";
                ArgsHEmitter(edl).emit(untrusted_dir);
                HEmitter(edl).emit_pop_u_h(untrusted_dir, prefix);
                if (!header_only)
                    CEmitter(edl).emit_pop_u_c(untrusted_dir, prefix);
            }            
        }
    }
