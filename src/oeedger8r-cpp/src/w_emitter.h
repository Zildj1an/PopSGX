...
class WEmitter
{
  ...
  public:
   ...
   void pop_emit(Function* f, bool ecall, const std::string& prefix = "")
    {
        ecall_ = ecall;
        has_deep_copy_out_ = has_deep_copy_out(edl_, f);
        std::string alloc_fcn;
        ...
        out() << "    "
              ...
              << "    /* Call " + other + " function. */";
              if (ecall)
                out() << "    if (is_pop_call(" + fcn_id + ",1)){";
              else
                out() << "    if (is_pop_call(" + fcn_id + ",0)){";
              /* Here, instead of calling oe_call_enclave_function(...) or
               * oe_call_host_function() we ask PopSGX to request the other 
               * side to execute this 
               */
              out() << " "
              << "        /* Request the other side to execute this call */"
              << "        _result = popsgx_call(\""+ call + "\",1," + fcn_id + ");"
              << " ";
              if (fcn_id.find("encrypt_block") != std::string::npos){
                    out() << ""
                    << "        if (!encrypt){"
                    << "            if (!user_block)__read_block();"
                    << "            strcpy(output_buf,user_block);"
                    << "        }";
              }
              out() << ""
              << "        goto done;"
