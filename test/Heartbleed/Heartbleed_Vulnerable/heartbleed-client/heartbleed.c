...
{
    int sock_desc = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
    ...
    if (1 != inet_pton(AF_INET, DEFAULT_IP, &server_addr.sin_addr))
    {
        printf("Invalid server IP address");
        return 0;
    }
    server_addr.sin_port = htons(atoi(DEFAULT_PORT_SERVER));
    ...
    printf("Connected to server via port %s\n",DEFAULT_PORT_SERVER);
    ...
    recv_hdr(sock_desc, &type, &version, &length);
    recv_data(sock_desc, length, buf);

    /* Let's just print the first 300... */
    hexdump(buf, 300);
